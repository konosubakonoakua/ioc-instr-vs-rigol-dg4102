...
class RigolDG4102Driver(Driver):
    ...
    def _safe_query(self, cmd, expected_type=None):
        ...
        with self.lock:
            try:
                # Small breathe time for instrument
                time.sleep(0.03)
                val = self.instr.query(cmd).strip().replace('"', "")
                
                # Desynchronization detection: 
                # If we expect a number but get an instrument keyword like 'INT', 'MAN', 'SIN'
                if expected_type == 'numeric':
                    try:
                        # Handle scientific notation and simple floats
                        float(val)
                    except ValueError:
                        logging.error(f"Sync error: Query '{cmd}' expected number but got '{val}'. Flushing buffer.")
                        self.instr.write("*CLS")
                        self.instr.clear()
                        return None
    ...
    def _sync_state(self, target_channel=None):
        """Poll the instrument with pacing to prevent buffer overflow."""
        if not self.connected:
            return
        ...
        with self.lock:
            for pv_name in self.pvdb:
                ...
                # Fetch value using internal read which calls safe_query
                self.read(pv_name)
                # Small delay between multiple queries in a burst
                time.sleep(0.01)
            ...
            if not self.connected:
                ...
                with self.lock:
                    try:
                        ...
                        # Tiered Polling: Full sync every 15 seconds
                        if time.time() - self.last_full_sync > 15:
                            self._sync_state()
                    except Exception as e:
    ...
    def _parse_reason(self, reason):
        ...
        elif reason.startswith("CH2:"): return 2, reason[4:]
        return None, reason
    ...
    def _get_scpi_cmd(self, channel, suffix, is_query=False):
        ...
        elif not is_query and suffix in {"TRIG_MANUAL"}: return "*TRG"
        elif not is_query and suffix in global_action_mapping: return global_action_mapping[suffix]
        return None
        ...
        if reason in self.pvdb and self.pvdb[reason].get("type") == "enum":
            ...
            try:
                if isinstance(value, str):
                    if "item " in value: value = int(value.replace("item ", ""))
                    elif value.upper() in enums: value = enums.index(value.upper())
                epics_value = int(value)
                scpi_value = enums[epics_value]
            except: return False

            if suffix == "IMPEDANCE": scpi_value = "50" if scpi_value == "50 Ohm" else "INF"
            ...
            if cmd:
                with self.lock:
                    ...
                    try:
                        ...
                        self.instr.query("*OPC?")
                        if reason == "SYSTEM_PRESET": self._sync_state()
                        elif reason == "COPY_1_TO_2": self._sync_state(target_channel=2)
                        elif reason == "COPY_2_TO_1": self._sync_state(target_channel=1)
                        elif reason == "PHASE_SYNC": self._sync_state()
    ...
    def read(self, reason):
        ...
        channel, suffix = self._parse_reason(reason)
        if channel:
            # Determine expected type for safe query
            expected = 'numeric'
            if reason in self.pvdb and self.pvdb[reason].get('type') == 'enum':
                expected = 'enum'
            ...
            if cmd:
                ...
                if reason in self.pvdb and self.pvdb[reason].get("type") == "enum":
                    ...
                    try:
                        ...
                        return index
                    except: return self.getParam(reason)
                else:
                    try:
